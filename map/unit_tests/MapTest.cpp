...
namespace mh
{
	namespace map
	{
		namespace test
		{
			...
			{
				const Map map(MapCoords(2, 4), emptyTile);
				EXPECT_EQ(MapCoords(2, 4), map.size());
			}

			TEST_F(MapTest, Empty_ShouldHaveProperTiles)
			{
				const Map map(MapCoords(2, 4), emptyTile);
				EXPECT_EQ(emptyTile, map.typeAt(MapCoords(0, 0)).id());
				EXPECT_EQ(emptyTile, map.typeAt(MapCoords(1, 3)).id());
			}

			TEST_F(MapTest, SingleTileSet_ShouldProperTileType)
			{
				Map map(MapCoords(2, 4), emptyTile);
				map.set(MapCoords(1, 2), someTile);
				EXPECT_EQ(someTile, map.typeAt(MapCoords(1, 2)).id());
			}
		}
	}
}
